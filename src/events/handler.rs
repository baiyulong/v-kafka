...
use crate::app::{
    App, ClusterFormField, InputMode, MessageInput, ProducerForm, View, AUTH_MECHANISMS,
};
...
async fn handle_normal(key: KeyEvent, app: &mut App) -> Result<()> {
    ...
    match &app.current_view {
        ...
        View::ProducerForm => {
            handle_producer_normal(key, app);
            Ok(())
        }
...
async fn connect_to_cluster(app: &mut App) -> Result<()> {
    ...
    match KafkaClient::new(&cluster) {
        Ok(client) => match fetch_cluster_metadata(&client.config, Duration::from_secs(8)) {
            Ok(meta) => {
                let msg = format!(
...
async fn handle_topic_list(key: KeyEvent, app: &mut App) -> Result<()> {
    ...
    match key.code {
        ...
        KeyCode::Enter if count > 0 => {
            let topic_name = app.filtered_topics()[app.list_cursor.min(count - 1)]
                .name
                .clone();
                    ...
                    Ok(Ok(wm)) => {
                            ...
                            .find(|t| t.name == topic_name)
                        {
                            for p in t.partitions.iter_mut() {
                                if let Some(&(_, low, high)) =
                                    app.watermarks.iter().find(|(id, _, _)| *id == p.id)
                                {
        ...
        KeyCode::Char('p') => {
            ...
            if count > 0 {
                let topic = app.filtered_topics()[app.list_cursor.min(count - 1)]
                    .name
                    .clone();
                app.producer_form = ProducerForm {
                    topic,
                    ..Default::default()
                };
            }
    ...
    match key.code {
        ...
        KeyCode::Char(']') | KeyCode::Right => {
            let next = app
                ...
                .last()
                .map(|m| m.offset + 1)
                .unwrap_or(app.messages_start_offset);
...
async fn handle_consumer_groups(key: KeyEvent, app: &mut App) -> Result<()> {
    ...
    match key.code {
        ...
        KeyCode::Enter if count > 0 => {
            let group = app.consumer_groups[app.list_cursor.min(count - 1)]
                .group_id
                .clone();
...
async fn load_group_offsets(app: &mut App) -> Result<()> {
    ...
    let partitions: Vec<(String, i32)> = app
        .metadata
        .topics
        .iter()
    ...
    let result =
        tokio::task::spawn_blocking(move || fetch_group_offsets(&cfg, &group_id, &partitions))
            .await;
    app.group_offsets_loading = false;
    match result {
        Ok(Ok(offsets)) => {
            // Only keep partitions with committed or non-zero high watermark
            app.consumer_group_offsets = offsets
                .into_iter()
            ...
            app.set_status(format!(
                ...
                app.consumer_group_offsets.len(),
                total_lag
            ));
        }
...
async fn handle_broker_info(key: KeyEvent, app: &mut App) -> Result<()> {
    match key.code {
        KeyCode::Down | KeyCode::Char('j') => {
            ...
            if n > 0 {
                app.list_cursor = (app.list_cursor + 1).min(n - 1);
            }
        }
            ...
            .await
            {
                ...
                Ok(Err(e)) => {
                    if let Some(idx) = app.cluster_form_edit_index {
                        ...
                        app.profile_manager
                            .profiles
                            .push(app.cluster_form.to_cluster_config());
                    }
                    app.profile_manager.save()?;
                    app.input_mode = InputMode::Normal;
                    app.navigate_back();
...
async fn handle_producer_editing(key: KeyEvent, app: &mut App) -> Result<()> {
    ...
    match key.code {
        ...
        KeyCode::BackTab => {
            app.producer_form.focused_field = if focused == 0 {
                field_count - 1
            } else {
                focused - 1
            };
        ...
        KeyCode::Backspace => {
            if let Some(s) = app.producer_form.current_str_mut() {
                s.pop();
            }
        }
        KeyCode::Char(c) => {
...
async fn send_message_action(app: &mut App) -> Result<()> {
    ...
    let headers: Vec<(String, Vec<u8>)> = headers_str
        .split(',')
        .filter_map(|h| {
                ...
                MessageInput::Timestamp => {
                    if let Ok(ts) = input.parse::<i64>() {
                        ...
                        match result {
                            Ok(Ok(msgs)) => {
                                app.set_status(format!(
                                    "Loaded {} messages from timestamp",
                                    msgs.len()
                                ));
                                if !msgs.is_empty() {
                                ...
                                app.messages = msgs;
                                app.selected_message_idx = if app.messages.is_empty() {
                                    None
                                } else {
                                    Some(0)
                                };
